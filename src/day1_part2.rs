    ...
    if let Ok(lines) = read_lines("day1.txt") {
        for line in lines.map_while(Result::ok) {
            let rotation = Rotation {
                direction: if line.starts_with('L') {
                    Direction::Left
                } else {
                    Direction::Right
                },
